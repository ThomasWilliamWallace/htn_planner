...
public:
    ...
    std::shared_ptr<BaseAction> action = std::make_shared<BaseAction>(Actions::useRoom); //stores the current action being attempted. This is held till the end of the frame, as it may be interrupted by another action.
    std::shared_ptr<BaseAction> lastAction = std::make_shared<BaseAction>(Actions::useRoom); //stores the last action taken. Useful for checking if hidden.
